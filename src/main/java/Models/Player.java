...
public class Player implements Serializable {
    ...
    public void setD_playerLog(String p_playerLogMessage, String p_logType) {
        ...
        switch (p_logType) {
            case Constants.ERROR_LOG_MSG:
                System.err.println(p_playerLogMessage);
                break;
            case Constants.LOG_MSG:
    ...
    public Order next_order() {
        if (Common.isCollectionEmpty(this.d_playerOrder)) {
            return null;
        }
    ...
    public void initDeployOrder(String p_command) {
        ...
        try {
            ...
            if (checkDeployArmyCount(this, l_armyCount)) {
                ...
                // queue
                this.setD_playerLog("Order Queued for player: " + this.d_playerName, Constants.LOG_MSG);
                
            }
        } catch (Exception l_exception) {
            this.setD_playerLog("Invalid command for deploy", Constants.ERROR_LOG_MSG);
        }
    }
    ...
    public void initAdvanceOrder(String p_commandEntered, GameState p_gameState) {
        try {
            if (p_commandEntered.split(" ").length == 4) {
                ...
                this.setD_playerLog("Invalid Arguments Passed For Advance Order", Constants.ERROR_LOG_MSG);
            }
            
        } catch (Exception l_e) {
            this.setD_playerLog("Invalid Advance Order Given", Constants.ERROR_LOG_MSG);
        }
    }
    ...
    public boolean areCountriesAdjacent(GameState p_gameState, String p_sourceCountry, String p_targetCountry) {
        ...
        if (!l_isAdjacent) {
            this.setD_playerLog("Advance order cannot be executed since target country : " + p_targetCountry
                    + " is not adjacent to source country : " + p_sourceCountry, Constants.ERROR_LOG_MSG);
        }
        return l_isAdjacent;
    }
    ...
    public void allocateCard() {
        if (!d_allowOnlyOneCardPerTurn) {
            ...
            this.d_playerOwnedCardList
                    .add(Constants.AVAILABLE_CARDS.get(l_random.nextInt(Constants.NUM_AVAILABLE_CARDS)));
        ...
        if (checkCardCommands(p_receivedCommand)) {
            switch (p_receivedCommand.split(" ")[0]) {
                case "airlift":
                    ...
                    if (l_newOrder.checkOrderValidity(p_gameState)) {
                        ...
                        this.setD_playerLog("Card Command Added to Queue for Execution Successfully!",
                                Constants.LOG_MSG);
                        p_gameState.updateLog(getD_playerLogMessage(), Constants.ORDER_EFFECT);
                    }
                    break;
                case "blockade":
                ...
                case "negotiate":
                    ...
                    break;
                default:
                    this.setD_playerLog("Invalid Command!", Constants.ERROR_LOG_MSG);
                    p_gameState.updateLog(getD_playerLogMessage(), Constants.ORDER_EFFECT);
                    break;
            }
        } else {
            this.setD_playerLog("Invalid Card Command Passed! Check Arguments!", Constants.ERROR_LOG_MSG);
        }
    }
}