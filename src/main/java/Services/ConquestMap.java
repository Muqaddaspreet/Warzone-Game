...
public class ConquestMap implements Serializable {
    ...
    public void readMapContent(List<String> p_fileContent, Map p_map, GameState p_gameState) {
        ...
        List<String> l_continentData = retrieveMetaData(p_fileContent, Constants.CONTINENT);
        List<String> l_countryData = retrieveMetaData(p_fileContent, Constants.COUNTRY);
        ...
        switch (p_switchParameter) {
            case Constants.CONTINENT:
                List<String> l_continentLines = p_fileLines.subList(
                        p_fileLines.indexOf(Constants.CONQUEST_CONTINENT) + 1,
                        p_fileLines.indexOf(Constants.CONQUEST_TERRITORIES) - 1);
                return l_continentLines;
            case Constants.COUNTRY:
                List<String> l_countryLines = p_fileLines
                        .subList(p_fileLines.indexOf(Constants.CONQUEST_TERRITORIES) + 1, p_fileLines.size());
    ...
    public void writeContinentMetadata(GameState p_gameState, FileWriter p_writer) throws IOException {
        p_writer.write(System.lineSeparator() + Constants.CONQUEST_CONTINENT + System.lineSeparator());
